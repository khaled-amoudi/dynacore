...
@section('toolbar')
    <x-BaseComponents.layout.breadcrumb :prev_pages="[
        'dashboard' => 'dashboard',
    ]" :current_page="$data['resource_name'] . ' trash list'" />
@endsection
                    ...
                    <div class="card shadow-sm mb-5 mb-xl-8">
                        <div class="card-header pt-5 pb-3">
                            <h3 class="card-title align-items-start flex-column">
                                <span
                                    class="card-label fw-bolder text-gray-700 fs-3 mb-1">{{ __($data['resource_name'] . ' trash list') }}</span>

                            </h3>

                            <div class="card-toolbar">

                                {{-- @if (isset($data['table_data']['configs']['with_multi_delete']) && $data['table_data']['configs']['with_multi_delete'] == true)
                                    @if (isset($data['table_data']['configs']['with_soft_delete']) && $data['table_data']['configs']['with_soft_delete'] == true)
                                    ...
                                    <thead>
                                        <tr class="fw-bold fs-6 text-gray-800 bg-light">
                                            @foreach ($data['columns'] as $th)
                                                <th class="fw-bolder py-7 text-capitalize">{{ __($th['name']) }}</th>
                                            @endforeach
                                        </tr>
                                    </thead>
                                    <tbody>
