<div id="kt_aside" class="aside aside-dark aside-hoverable" data-kt-drawer="true" data-kt-drawer-name="aside"
    ...
    <div class="aside-logo flex-column-auto" id="kt_aside_logo">
        ...
        <a href="{{ route('dashboard') }}">
            <img alt="Logo" src="{{ asset('cms/assets/media/custom/logos/dynacore-dark.png') }}"
                class="h-40px logo" />
        </a>
        <!--end::Logo-->
        <!--begin::Aside toggler-->
                    ...
                    @if (isset($item['menu_label']))
                        <div class="menu-item">
                            <div class="menu-content pt-8 pb-2">
                                <span
                                    class="menu-section text-muted text-uppercase fs-8 ls-1">{{ __($item['menu_label']) }}</span>
                            </div>
                        </div>
                    @elseif(isset($item['menu_title']))
                        @can($item['permission'])
                            <div data-kt-menu-trigger="click"
                                class="menu-item @if (Route::is($item['active'])) here show @endif menu-accordion">
                                <span class="menu-link py-4">
                                    <span class="menu-icon">
                                        <!--begin::Svg Icon | path: icons/duotune/general/gen002.svg-->
                                        <span class="svg-icon svg-icon-2">
                                            {{-- <i class="{{ $item['menu_title_icon'] }}"></i> --}}
                                            ...
                                            {!! $item['menu_title_icon'] !!}
                                        </span>
                                        <!--end::Svg Icon-->
                                    </span>
                                    <span class="menu-title">{{ __($item['menu_title']) }}</span>
                                    <span class="menu-arrow"></span>
                                </span>
                                <div class="menu-sub menu-sub-accordion">
                                    @foreach ($item['menu_title_list'] as $list_item)
                                        @can($list_item['permission'])
                                            <div class="menu-item">
                                                <a class="menu-link @if (Route::is($list_item['active'])) active @endif"
                                                    href="{{ route($list_item['route']) }}">
                                                    <span class="menu-bullet">
                                                        <span class="bullet bullet-dot"></span>
                                                    </span>
                                                    <span class="menu-title">{{ __($list_item['label']) }}</span>
                                                </a>
                                            </div>
                                        @endcan
                                    @endforeach
                        ...
                        @can($item['permission'])
                            <li class="menu-item" aria-haspopup="true">
                                <a href="{{ route($item['route']) }}"
                                    class="menu-link @if (Route::is($item['active'])) active @endif py-4">
                                    ...
                                    <span class="menu-text">{{ __($item['label']) }}</span>
                                </a>
                            </li>
                        @endcan
                    @endif
